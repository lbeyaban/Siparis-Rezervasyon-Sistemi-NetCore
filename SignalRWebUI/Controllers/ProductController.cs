...
namespace SignalRWebUI.Controllers
{
	public class ProductController : Controller
	{
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.DeleteAsync($"https://localhost:7022/api/Product/{id}");
			if(responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
			}
		...
		public async Task<IActionResult> UpdateProduct(int id)
		{
			...
			var responseMessage1 = await client1.GetAsync("https://localhost:7022/api/Category");
			...
			if(responseMessage.IsSuccessStatusCode)
			{
				...
				{
					if(item.Value == values.CategoryID.ToString())
					{
						item.Selected = true;
					}
				}
		...
		public async Task<IActionResult> UpdateProduct(UpdateProductDto updateProduct)
		{
			...
			var responseMessage = await client.PutAsync("https://localhost:7022/api/Product", content);
			if(responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
			}
